...
public abstract class GA implements Runnable
{
	...
	public void run()
	{
		...
		// Si la classe a evoluer est un horaire, on va creer une feuille excel representant l'horaire
		if(this instanceof com.softtechdesign.ga.examples.GAHoraire)
		{
			double bestFitness = this.chromosomes[this.bestFitnessChromIndex].fitness;
			int cptFeuilles = 0;
			Chromosome[] c = new Chromosome[5];
			boolean out = false;
			for(int k=0; k<populationDim; k++)
				if (this.chromosomes[k].fitness >= bestFitness-0.05 && cptFeuilles < 5)
				{   
					for(int l=0; l<cptFeuilles; l++)
						if(!this.chromosomes[k].equals(c[l]))
							out = true;
					if(!out)
					{
						c[cptFeuilles] = this.chromosomes[k];
						cptFeuilles++;
						// Recuperation du meilleur chromosome
						ChromStrings chromosome = (ChromStrings)this.chromosomes[k];
						// Creation d'une instance dont on pourra recuperer les parametres originaux
						GAHoraire g = (GAHoraire)this;
						// Creation de la feuille excel
						try
						{
							WritableWorkbook workbook = Workbook.createWorkbook(new File("output"+k+".xls"));
							WritableSheet sheet = workbook.createSheet("Horaire", 0);

							// Tableau des jours a afficher
							String[] jours = {"Lundi","Mardi","Mercredi","Jeudi","Vendredi","Samedi","Dimanche"};

							// Recuperation du tableau des noms des employes
							String[] nomsEmployes = g.getIndiv();

							// Recuperation des scores des jours de la semaine
							double[] scoreJours = g.getScoreJours();
							double[] score = new double[nomsEmployes.length];
							for(int i=0; i<nomsEmployes.length; i++)
								score[i] = 0;

							// Affichage du nombre de jours concernes par l'horaire
							for(int i=0; i<chromosomeDim; i++)
							{Label label = new Label(i+1, 0, jours[i%7]);
							sheet.addCell(label);}

							// Recuperation du tableau des places non dsiponible
							int[][] tabND = g.getPlacesND();

							Label label;
							jxl.write.Number label2;
							int cpt = 0;
							boolean nd = false;

							// Ecriture des jours travailles, non-dispos, nb jours, etc...
							for(int j=0; j<nomsEmployes.length; j++)
							{
								for(int i=0; i<chromosomeDim; i++)
								{
									// Detection d'un jour non-disponible
									if(tabND[j][i] == 1)
										nd = true;
									// Detection d'un jour de travail fourni par l'algorithme
									if(chromosome.getGene(i).contains(nomsEmployes[j]))
									{
										// Si le jour n'etait pas non-dispo, on met un "T"
										if(!nd)
											label = new Label(i+1, j+1, "T");
										// Sinon, on signale que le jour ne devrait pas etre travaille
										else
											label = new Label(i+1, j+1, "T(nd)");
										sheet.addCell(label);
										cpt++;
										score[j] += scoreJours[i%7];
									}
									// Si le jour n'est pas attribue et qu'il est non-dispo, on l'ecrit
									else if(nd)
									{
										label = new Label(i+1, j+1, "ND");
										sheet.addCell(label);
									}
									nd = false;
								}
								// Ecriture du nombre de jours travailles
								label2 = new jxl.write.Number(chromosomeDim + 1, j+1, cpt);
								sheet.addCell(label2);
								label2 = new jxl.write.Number(chromosomeDim + 2, j+1, score[j]/cpt);
								sheet.addCell(label2);
								cpt = 0;
							}

							for(int i=0; i<nomsEmployes.length; i++)
							{label = new Label(0, i+1, nomsEmployes[i]);
							sheet.addCell(label);}
							label2 = new jxl.write.Number(0, 0, this.chromosomes[k].fitness);
							sheet.addCell(label2);
							workbook.write();
							workbook.close();
	 ...
	 */
	public synchronized int evolve()
	{
		int iGen;
	...
	protected double getAvgDeviationAmongChroms()
	{
		...
		for (int iGene = 0; iGene < this.chromosomeDim; iGene++)
		{
			...
			{
				double bestFitGene =
					((ChromInt)this.chromosomes[this.bestFitnessChromIndex]).getGene(iGene);
				for (int i = 0; i < populationDim; i++)
				{
					double thisGene = ((ChromInt)this.chromosomes[i]).getGene(iGene);
					if (thisGene != bestFitGene)
						devCnt++;
				}
			}
